...
ffmpeg -i $1  -vf "select=eq(pict_type\,I)" -vsync vfr -q:v 2 scenes/${videobasename}~%04d.jpg
...
ls -1 scenes/${videobasename}~*.jpg | xargs -n 1 basename > metadata/${videobasename}_keyframe_thumbnails.txt
...
while read -r thumbnail index; do newIndex=$(echo $index - 1 | bc); mv -- "scenes/$thumbnail" "scenes/$videobasename~$newIndex.jpg"; done < metadata/${videobasename}_keyframes.txt
# 
# Cleanup
